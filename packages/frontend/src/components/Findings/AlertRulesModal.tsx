/**
 * AlertRulesModal Component
 * Modal interface for managing alert rules
 * Extracted from AlertRulesPage for use in FindingsCenter
 */
...
export default function AlertRulesModal({ isOpen, onClose }: AlertRulesModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-[#1A1A1A] border border-[#2D2D2D] rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="sticky top-0 bg-[#1A1A1A] border-b border-[#2D2D2D] p-6 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold text-white flex items-center gap-2">
              <Bell size={20} />
              Gestionar Reglas de Alertas
            </h2>
            <p className="text-sm text-[#A0A0A0] mt-1">
              Crea reglas automáticas para recibir notificaciones cuando se cumplen ciertas condiciones
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Form */}
          {showForm && (
            <div className="bg-[#111] border border-[#2D2D2D] rounded-lg p-6 space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="bg-[#111] border border-[#2D2D2D] rounded-lg p-8 text-center">
            ...
            <div className="space-y-3">
              {rules.map((rule) => (
                <div
                  key={rule.id}
                  className="bg-[#111] border border-[#2D2D2D] rounded-lg p-4 hover:border-[#4B5563] transition-colors"
                >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-[#666666] space-y-1">
                        ...
                        {rule.triggeredCount > 0 && (
                          <p>
                            ✅ Disparos:{' '}
                            <span className="text-white font-semibold">{rule.triggeredCount}</span>
